...
model TestPipe_tflite
  ...
  Modelica.Fluid.Pipes.DynamicPipe pipe(
    ...
    redeclare replaceable model HeatTransfer = TFLite.NNHeatTransfer_tflite) annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
  Modelica.Fluid.Sources.Boundary_pT sink(redeclare package Medium = Medium, nPorts=1) annotation (Placement(transformation(extent={{80,-10},{60,10}})));
  ...
  Modelica.Thermal.HeatTransfer.Sources.FixedHeatFlow fixedHeatFlow[pipe.n](each Q_flow=1000) annotation (Placement(transformation(extent={{-40,20},{-20,40}})));
equation
  connect(source.ports[1], pipe.port_a) annotation (Line(points={{-60,0},{-10,0}}, color={0,127,255}));
  connect(fixedHeatFlow.port, pipe.heatPorts) annotation (Line(points={{-20,30},{0.1,30},{0.1,4.4}}, color={191,0,0}));
  connect(sink.ports[1], pipe.port_b) annotation (Line(points={{60,0},{10,0}}, color={0,127,255}));
  annotation (
