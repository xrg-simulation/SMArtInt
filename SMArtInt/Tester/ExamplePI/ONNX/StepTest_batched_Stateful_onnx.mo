...
model StepTest_batched_Stateful_onnx
  extends Modelica.Icons.Example;

  Blocks.EvaluateStatefulRecurrentNeuralNet controller(
    pathToAIModel=Modelica.Utilities.Files.loadResource("modelica://SMArtInt/Resources/ExampleNeuralNets/PIController/PI_stateful.onnx"),
    samplePeriod=10,
    batchSize=3,
...
equation
  connect(step.y, controller.u[1, 1]) annotation (Line(points={{-19,40},{0,40},{0,0},{18,0},{18,-0.666667}},
                                                                                                    color={0,0,127}));
  connect(const.y, controller.u[2, 1]) annotation (Line(points={{-19,0},{18,0}}, color={0,0,127}));
  connect(step1.y, controller.u[3, 1]) annotation (Line(points={{-19,-40},{0,-40},{0,0},{18,0},{18,0.666667}},
                                                                                                     color={0,0,127}));
...
end StepTest_batched_Stateful_onnx;
