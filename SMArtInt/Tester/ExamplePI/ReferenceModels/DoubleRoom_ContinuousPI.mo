...
model DoubleRoom_ContinuousPI
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor heatCapacitor(C=10000, T(
      displayUnit="K",
      start=280,
      fixed=true)) annotation (Placement(transformation(extent={{-6,72},{14,92}})));
  ...
  Modelica.Fluid.Sensors.TemperatureTwoPort temperature(redeclare package Medium = Medium) annotation (Placement(transformation(extent={{56,10},{76,-10}})));
  Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(
    table=[0.0,293.15,50/3600; 1,303.15,50/3600; 2,308.15,50/3600; 3,323.15,50/3600; 4,293.15,50/3600; 5,308.15,25/3600; 6,323.15,25/3600; 7,323.15,25/3600; 8,293.15,25/3600; 9,293.15,25/3600],
    smoothness=Modelica.Blocks.Types.Smoothness.ConstantSegments,
    timeScale=timeScale) annotation (Placement(transformation(extent={{96,-74},{76,-54}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature lossHeatFlow annotation (Placement(transformation(extent={{70,62},{50,82}})));
  Modelica.Blocks.Noise.UniformNoise lossNoise(
    ...
    startTime=1000,
    y_min=280,
    y_max=280) annotation (Placement(transformation(extent={{100,62},{80,82}})));
  Modelica.Blocks.Math.Add calcCntrlDev(k1=-1, k2=1) annotation (Placement(transformation(extent={{54,-50},{34,-30}})));

  Modelica.Thermal.HeatTransfer.Components.ThermalResistor thermalResistor(R=0.1) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=0,
        origin={30,72})));
...
equation
  ...
  connect(room_1.ports[2], volumeFlowRate.port_a) annotation (Line(points={{-27,10},{-30,10},{-30,0},{-20,0}}, color={0,127,255}));
  ...
  connect(controller.u, calcCntrlDev.y) annotation (Line(points={{18.6,-40},{33,-40}}, color={0,0,127}));
  ...
  connect(controller.y, controlHeatFlow.Q_flow) annotation (Line(points={{-2.4,-40},{-80,-40},{-80,52},{-72,52}}, color={0,0,127}));
  annotation (
