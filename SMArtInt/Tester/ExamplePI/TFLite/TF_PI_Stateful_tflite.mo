...
model TF_PI_Stateful_tflite
  ...
  Blocks.EvaluateStatefulRecurrentNeuralNet controller(samplePeriod=10, pathToAIModel=Modelica.Utilities.Files.loadResource("modelica://SMArtInt/Resources/ExampleNeuralNets/PIController/PI_stateful.tflite")) annotation (Placement(transformation(extent={{-10,-10},{10,10}})));
equation

  connect(u, controller.u[1, 1]) annotation (Line(points={{-120,0},{-12,0}}, color={0,0,127}));
  connect(controller.y[1, 1], y) annotation (Line(points={{11,0},{110,0}}, color={0,0,127}));
