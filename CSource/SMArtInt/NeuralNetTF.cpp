...
//    mp_modelicaUtilityHelper->ModelicaMessage("SMArtInt: Destructor TFLite Neural Network\n");
...
void TfLiteNeuralNet::loadAndInit(const char* tfliteModelPath)
{
    ...
    if (!mp_model) {
        std::string message = Utils::string_format("SMArtInt: Model not found - check path: %s",
                                                   m_tfliteModelPath);
    ...
    if (mp_tfdll->interpreterGetOutputTensorCount(mp_interpreter) != 1) {
        if (mp_timeStepMngmt->isActive()) {
            if (mp_tfdll->interpreterGetOutputTensorCount(mp_interpreter) !=
                ...
                mp_modelicaUtilityHelper->ModelicaError("SMArtInt: Stateful handling can only be done if model has "
                                             ...
                                             unsigned int outputLength)
{
    ...
    if (m_nInputEntries != inputLength) {
        std::string message = Utils::string_format("SMArtInt: Wrong input length: in the interface were %i "
    ...
    if (m_nOutputEntries != outputLength) {
        std::string message = Utils::string_format("SMArtInt: Wrong output length: in the interface were %i "
                                                   "entries defined, whereas in current function call %i is specified!",
...
void TfLiteNeuralNet::setInputCastFunction(TfLiteTensor* tensor)
{
    switch (mp_tfdll->tensorType(tensor)) {
        case kTfLiteFloat32:
            ...
            break;
        default:
            mp_modelicaUtilityHelper->ModelicaError("Could not convert input data - SMArtInt currently only supports TFLite models using floats");
            break;
    }
}
...
void TfLiteNeuralNet::setOutputCastFunction(const TfLiteTensor* tensor)
{
    switch (mp_tfdll->tensorType(tensor)) {
        case kTfLiteFloat32:
            ...
            break;
        default:
            mp_modelicaUtilityHelper->ModelicaError("Could not convert output data - SMArtInt currently only supports TFLite models using floats");
            break;
    }
}
